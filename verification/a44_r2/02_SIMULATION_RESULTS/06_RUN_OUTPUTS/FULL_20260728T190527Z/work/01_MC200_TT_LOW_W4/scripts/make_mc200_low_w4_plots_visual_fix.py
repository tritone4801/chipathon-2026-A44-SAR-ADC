...
def transition_plot(
    ...
    rows = [
        {
            "mismatch_seed": int(row["mismatch_seed"]),
            "historical_w0_sndr_db": float(row["historical_w0_sndr_db"]),
            "new_w4_sndr_db": float(row["new_w4_sndr_db"]),
            "delta_sndr_db": float(row["delta_sndr_db"]),
            "historical_w0_code_exact": truth(row["historical_w0_code_exact"]),
        }
        for row in transition
    ]
    x = np.asarray([row["historical_w0_sndr_db"] for row in rows])
    y = np.asarray([row["new_w4_sndr_db"] for row in rows])
    limits = (
    ...
    colors = [
        COLORS["blue"] if row["historical_w0_code_exact"] else COLORS["orange"]
        for row in rows
    ]
    ...
    ax.set_xlim(*limits)
    ax.set_ylim(*limits)
    ax.set_xlabel("Historical W0 SNDR [dB]")
    ax.set_ylabel("New W4 SNDR [dB]")
    ax.set_title("LOW MC200 method transition\nW0 versus FAST64_SS_W4")
    return fig, rows
...
def spectra_plot(
    ...
    for axis, (role, seed) in zip(axes.flat, selected):
        ...
        positive_rows = [
            row for row in spectrum if float(row["power_linear"]) > 0.0
        ]
        exact_zero_rows = [
            row for row in spectrum if float(row["power_linear"]) == 0.0
        ]
        exact_zero_bin_count += len(exact_zero_rows)
        finite = [
        ...
        frequencies = np.asarray(
            [float(row["freq_hz"]) / 1e6 for row in positive_rows]
        )
        magnitudes = np.asarray(
            [float(row["magnitude_db"]) for row in positive_rows]
        )
        ...
        for row in exact_zero_rows:
            frequency = float(row["freq_hz"]) / 1e6
            axis.scatter(
                frequency,
                ymin,
                marker="v",
            ...
            axis.annotate(
                "−∞",
                (frequency, ymin),
                xytext=((-2 if frequency >= 0.99 else 0), 5),
                textcoords="offset points",
                ha=("right" if frequency >= 0.99 else "center"),
                va="bottom",
                fontsize=7,
                color=COLORS["blue"],
            )
            ...
            source.append(
                {
                    ...
                    **row,
                    "display_class": (
                        "EXACT_ZERO_NEGATIVE_INFINITY"
                        if float(row["power_linear"]) == 0.0
                        else "FINITE_NONZERO"
                    ),
                    "plotted_magnitude_db": (
                        "-inf"
                        if float(row["power_linear"]) == 0.0
                        else row["magnitude_db"]
                    ),
                }
            )
    ...
    fig.tight_layout()
    fig._mc200_exact_zero_bin_count = exact_zero_bin_count
...
def main() -> int:
    ...
    inventory.append(save_figure("low_w0_vs_w4_sndr", fig, rows))
    fig, rows, spectrum_ymin = spectra_plot(master, representatives, retained)
    exact_zero_bin_count = int(getattr(fig, "_mc200_exact_zero_bin_count", 0))
    ...
    write_json_atomic(
        ROOT / "config/mc200_low_w4_plot_resolved_ranges.json",
        {
            ...
            "representative_spectrum_exact_zero_bin_count": exact_zero_bin_count,
            "exact_zero_bins_rendered_as_negative_infinity_markers": True,
