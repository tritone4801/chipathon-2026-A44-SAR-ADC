...
| --- | ---: | --- |
| 100-sample independent Monte Carlo mismatch dynamic simulation at the typical-typical process corner, 3.3 volts, 27 degrees Celsius, using the low differential-input band and steady-state frames 4 through 67 | **100/100** | Formal audit passed: 100/100 hard-dynamic passes and 100/100 signal-to-noise-budget passes, with zero protocol, Parseval, or clipping failures; minimum/mean/maximum signal-to-noise-and-distortion ratio is 47.993/48.927/49.890 decibels and effective number of bits is 7.680/7.835/7.995 bits |
| Full 255-transition static transfer-curve simulation | **255/255** | Representative preselected typical-corner seed 98 passed all 255 transitions, with zero missing codes and zero nonmonotonic transitions; maximum absolute differential nonlinearity is 0.109557 least-significant-bit units and maximum absolute endpoint-fit integral nonlinearity is 0.133033 least-significant-bit units. This is a representative-seed result, not yield, worst-case, parasitic-extraction pairing, or signoff |
| Three-corner process-voltage-temperature dynamic simulation using 20 selected Monte Carlo mismatch samples per corner | **60/60** | All 60 paired typical-typical, slow-slow, and fast-fast records passed the system specification with zero protocol failures; mean signal-to-noise-and-distortion ratio is 48.928/46.270/48.760 decibels for those corners, respectively, with slow-slow as the limiting corner. This selected diagnostic campaign uses fixed typical-corner behavioral successive-approximation-register timing; it is not yield, full-logic process-voltage-temperature qualification, parasitic extraction, or signoff |
| Circuit Automatic Characterization Engine package preflight | **Planned** | No Circuit Automatic Characterization Engine package preflight result is included in `PEX_GOLDEN_BASELINE`; the available preflight records qualify individual simulation campaigns and do not complete this entry |
